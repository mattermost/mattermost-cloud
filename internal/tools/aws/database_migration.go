...
func (d *RDSDatabaseMigration) Restore(logger log.FieldLogger) (string, error) {
	...
	if err != nil && !IsErrorCode(err, rds.ErrCodeDBClusterNotFoundFault) {
		err := newErrorWrap(&masterClusterID, &replicaClusterID, err)
		logger.WithError(err).Error("Restoring RDS database migration")
		return "", err
	}

	if dbClusterOutput != nil {
	...
	if err != nil && !IsErrorCode(err, rds.ErrCodeDBInstanceNotFoundFault) {
		err = newErrorWrap(&masterClusterID, &replicaClusterID, err)
		logger.WithError(err).Error("Restoring RDS database migration")
		return "", err
	}

	if dbInstanceOutput != nil {
