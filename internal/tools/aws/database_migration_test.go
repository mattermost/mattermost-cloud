...
func (a *AWSTestSuite) TestDatabaseRDSMigrationSetup() {
	...
	gomock.InOrder(
		...
		a.SetAuthorizeSecurityGroupIngress("Ingress Traffic from replica RDS instance cloud-id000000000000000000000000a-migration-master").
			Return(nil, nil).
			Times(1),
	)
...
func (a *AWSTestSuite) TestDatabaseRDSMigrationSetupError() {
	...
	a.Assert().Equal("unabled to restore RDS master id cloud-id000000000000000000000000a-master "+
		"to RDS replica id cloud-id000000000000000000000000a-migration-master: invalid group id", err.Error())
...
func (a *AWSTestSuite) TestDatabaseRDSMigrationSetupSGNotFoundError() {
	...
	a.Assert().Equal("unabled to restore RDS master id cloud-id000000000000000000000000a-master to RDS replica id "+
		"cloud-id000000000000000000000000a-migration-master: security group for RDS DB instance "+
...
func (a *AWSTestSuite) TestDatabaseRDSMigrationTeardownError() {
	...
	a.Assert().Error(err)
	a.Assert().Equal("unabled to restore RDS master id cloud-id000000000000000000000000a-master to RDS replica id "+
		"cloud-id000000000000000000000000a-migration-master: not enough permissions to revoke ingress rule", err.Error())
	a.Assert().Equal("", status)
}
...
func (a *AWSTestSuite) TestDatabaseRDSMigrationTeardownSGNotFoundError() {
	...
	a.Assert().Equal("unabled to restore RDS master id cloud-id000000000000000000000000a-master to RDS replica id "+
		"cloud-id000000000000000000000000a-migration-master: security group for RDS DB instance "+
		"cloud-id000000000000000000000000a-migration-master not found", err.Error())
	a.Assert().Equal("", status)
}
