...
func rdsDatabaseProvision(installationID string, logger log.FieldLogger) error {
	...
	rdsSecret, err := a.secretsManagerEnsureRDSSecretCreated(awsID)
	if err != nil {
		return err
	}
...
func rdsDatabaseTeardown(installationID string, keepData bool, logger log.FieldLogger) error {
	...
	if !keepData {
		...
		logger.WithField("rds-database-name", awsID).Info("AWS RDS database deleted")
	} else {
		logger.WithField("s3-bucket-name", awsID).Info("AWS RDS database was left intact due to the keep-data setting of this server")
	}
