...
func (sqlStore *SQLStore) GetStateChangeEvents(filter *model.StateChangeEventFilter) ([]*model.StateChangeEventData, error) {
	...
	if filter.Paging.PerPage != model.AllPerPage { // nolint: staticcheck
		query = query.
			Limit(uint64(filter.Paging.PerPage)).                      // nolint: staticcheck
			Offset(uint64(filter.Paging.Page * filter.Paging.PerPage)) // nolint: staticcheck
	}
